...
// ⚠️ 例外：新增一支被 import 的 JS 模組時一定要改（v2.56 教訓）——否則背景更新進快取的新 main.js
//    會 import 一支從沒被快取過的檔，下次離線整個 module graph 載入失敗、頁面空白。

const CACHE_NAME = 'sv2-runtime-v11';   // v2.56：科目列（首頁一鍵換科），舊快取作廢。
                                       // v2.55：會考題課次改用謙恩的課本版本，舊快取作廢。
                                       // v2.51：社會科會考題，舊快取作廢。
