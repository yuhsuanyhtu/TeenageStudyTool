...
export function startMatchMode({ root, words, onComplete }) {
  // 去重：同 zh 只取第一個
  const seen = new Set();
  const candidates = [];
  ...
  const round = shuffle(candidates).slice(0, Math.min(PAIRS_PER_ROUND, candidates.length));
  // 左欄是英文（按原順序），右欄是中文（打亂）
  ...
  function markWrong() {
    const enBtn = root.querySelector(`.match-card[data-side="en"][data-idx="${state.selectedEn}"]`);
    const zhBtn = root.querySelector(`.match-card[data-side="zh"][data-idx="${state.selectedZh}"]`);
    if (enBtn) enBtn.classList.add('wrong');
    if (zhBtn) zhBtn.classList.add('wrong');
  }
