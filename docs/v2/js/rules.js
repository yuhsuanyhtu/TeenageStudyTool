...
export function renderRules(root, onBack) {
  ...
  root.innerHTML = `
    ...
    <div class="card">
      <h3>🎯 會考題（v2.50 新增）</h3>
      <p>每一課的題型選單裡有「🎯 會考題」：歷屆國中教育會考的<b>真題</b>，只出用國小＋國一程度的字就看得懂的題目（二年級會再加上 A2 的字）。一卷最多 10 題，簡單的先來。</p>
      <p class="muted small">· 答對一題 <b>$${(st.cfg && st.cfg['rate.hk.per']) ?? 2}</b>，<b>同一題只付一次</b>（題目是固定的，背答案不算數）。<br>
      · 答錯的題 14 天內再答對不給錢，過了 14 天再答對就能領。<br>
      · 不乘連勝、沒有基礎獎金、不算打卡；算在英文的每日上限裡。<br>
      · 答錯會告訴你正確答案、這題考什麼、回去複習哪一課。看不懂可以按「跳過」，還沒教過的可以按「這題還沒教過」，以後就不會再出。<br>
      · 🌏 社會（歷史・地理・公民）、🔬 自然（生物・理化・地科）、📐 數學（只有選擇題）、📜 國文從首頁進去，選學校上到的那一課。各科有自己的每日上限。<br>
      · 按了「這題還沒教過」的題，等你選到更後面的課才會再出現。</p>
    </div>
